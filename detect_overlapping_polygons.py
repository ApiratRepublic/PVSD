...
def classify_pair(
    ...
    return {
        ...
        "ra": ra, "rb": rb,
        "reason_tmpl": (
            "OID {CHECK_OID} และ {COMPARE_OID} ซ้อนทับกัน {overlap_area:.2f} m² "
...
def build_records(gdb: str, fc: str, pairs: list[dict]) -> list[dict]:
    ...
    for p in pairs:
        ...
        # ตรงนี้มีเพื่อเช็คว่าคีย์ "a" และ "b" ใน cls มีค่าเป็น "a" หรือ "b" เสมอ (ไม่ใช่ None)
        # เพราะเราต้องใช้คีย์เหล่านี้ในการดึงค่า OID และพื้นที่สำหรับ CHECK และ COMPARE 
        # ใส่ในฟิลด์ต่าง ๆ ของรายงาน
        oid_map  = {"a": a,             "b": b}
